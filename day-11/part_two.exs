defmodule DumboOctopus do
  ...
  def run_steps(energy_levels) do
    %{flash_count: flash_count} =
      Enum.reduce(1..@steps_to_run, %{energy_levels: energy_levels, flash_count: 0}, fn step, acc -> run_step(acc, step) end)

    flash_count
  end

  def run_step(%{energy_levels: energy_levels, flash_count: flash_count}, step) do
    {updated_energy_levels, new_flash_count} =
      energy_levels
      |> increment_energy_levels()
      |> propagate_flashes(step)
  ...
  def propagate_flashes(energy_levels, step) do
    propagate_flashes(energy_levels, step, [], 0)
  end

  def propagate_flashes(_, step, _, 100) do 
    raise "All flashed on step #{step}..."
  end

  def propagate_flashes(energy_levels, step, _flashes, flash_count) do
    case find_new_flashes(energy_levels) do
      {updated_energy_levels, []} -> {updated_energy_levels, flash_count}
      {updated_energy_levels, new_flashes} -> propagate_flashes(updated_energy_levels, step, new_flashes, flash_count + length(new_flashes))
    end
  end
